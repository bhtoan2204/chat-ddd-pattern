package repos

import (
	"context"

	"wechat-clone/core/modules/account/domain/aggregate"
)

//go:generate mockgen -package=repos -destination=session_aggregate_repo_mock.go -source=session_aggregate_repo.go
type SessionAggregateRepository interface {
	Load(ctx context.Context, sessionID string) (*aggregate.SessionAggregate, error)
	Save(ctx context.Context, session *aggregate.SessionAggregate) error
	ListByAccountID(ctx context.Context, accountID string) ([]*aggregate.SessionAggregate, error)
}

type SessionRepository = SessionAggregateRepository
