...
func (h *callbackGoogleHandler) Handle(c *gin.Context) (interface{}, error) {
	ctx := c.Request.Context()
	logger := logging.FromContext(ctx)
	var request in.CallbackGoogleRequest
	...
	result, err := h.callbackGoogle.Dispatch(ctx, &request)
	if err != nil {
		logger.Errorw("CallbackGoogle failed", zap.Error(err))
		return nil, stackErr.Error(err)
	}
	return result, nil
}
