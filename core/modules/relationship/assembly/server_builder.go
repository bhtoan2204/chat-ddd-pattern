...
func buildHTTPServer(_ context.Context, appContext *appCtx.AppContext) (http.HTTPServer, error) {
	...
	listIncomingFriendRequests := cqrs.NewDispatcher(relationshipquery.NewListIncomingFriendRequests(appContext, relationshipReadRepos, relationshipRepos.RelationshipAccountRepository()))
	listOutgoingFriendRequests := cqrs.NewDispatcher(relationshipquery.NewListOutgoingFriendRequests(appContext, relationshipReadRepos, relationshipRepos.RelationshipAccountRepository()))
	unfriendUser := cqrs.NewDispatcher(relationshipcommand.NewUnfriendUser(appContext, relationshipRepos))
	listFriends := cqrs.NewDispatcher(relationshipquery.NewListFriends(appContext, relationshipReadRepos, relationshipRepos.RelationshipAccountRepository()))
	...
	listFollowers := cqrs.NewDispatcher(relationshipquery.NewListFollowers(appContext, relationshipReadRepos, relationshipRepos.RelationshipAccountRepository()))
	listFollowing := cqrs.NewDispatcher(relationshipquery.NewListFollowing(appContext, relationshipReadRepos, relationshipRepos.RelationshipAccountRepository()))
	...
	listBlockedUsers := cqrs.NewDispatcher(relationshipquery.NewListBlockedUsers(appContext, relationshipReadRepos, relationshipRepos.RelationshipAccountRepository()))
	getRelationshipStatus := cqrs.NewDispatcher(relationshipquery.NewGetRelationshipStatus(appContext, relationshipReadRepos))
	getMutualFriends := cqrs.NewDispatcher(relationshipquery.NewGetMutualFriends(appContext, relationshipReadRepos, relationshipRepos.RelationshipAccountRepository()))
