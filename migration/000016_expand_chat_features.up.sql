ALTER TABLE rooms ADD direct_key VARCHAR(2048);

ALTER TABLE rooms ADD pinned_message_id VARCHAR(1024);

CREATE UNIQUE INDEX uq_rooms_direct_key ON rooms(direct_key);

ALTER TABLE room_members ADD last_delivered_at TIMESTAMPTZ;

ALTER TABLE room_members ADD last_read_at TIMESTAMPTZ;

ALTER TABLE messages ADD message_type VARCHAR(50) DEFAULT 'text' NOT NULL;

ALTER TABLE messages ADD reply_to_message_id VARCHAR(1024);

ALTER TABLE messages ADD forwarded_from_message_id VARCHAR(1024);

ALTER TABLE messages ADD file_name VARCHAR(1024);

ALTER TABLE messages ADD file_size BIGINT;

ALTER TABLE messages ADD mime_type VARCHAR(255);

ALTER TABLE messages ADD object_key VARCHAR(2048);

ALTER TABLE messages ADD edited_at TIMESTAMPTZ;

ALTER TABLE messages ADD deleted_for_everyone_at TIMESTAMPTZ;
